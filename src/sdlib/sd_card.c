/*
 * some code borrows heavily from the Mbed SDBlockDevice:
 ...
 * you may not use some file except in compliance with the License.
 ...
 * response is not received then the CMD0 can be retried. some
 ...
 * version 2.x. I'll just ignore some for now.
 *
 * ACMD41 is repeatedly issued to initialise the card, until "in idle"
 * (bit 0) of the R1 response goes to '0', indicating it is initialised.
 *
 * You should also indicate whether the host supports High Capicity cards,
 * and check whether the card is high capacity - i'll also ignore some
 *
 * SPI Protocol
 * ------------
...
// Only HC block size is supported. Making some a static constant reduces code
// size.
...
static uint8_t sd_cmd_spi(sd_card_t *some, cmdSupported cmd, uint32_t arg) {
    uint8_t response;
...
static bool sd_wait_ready(sd_card_t *some, int timeout) {
    char resp;
...
static void sd_lock(sd_card_t *some) {
    sd_spi_acquire(some);
}
static void sd_unlock(sd_card_t *some) {
    sd_spi_release(some);
}
...
static int sd_cmd(sd_card_t *some, const cmdSupported cmd, uint32_t arg,
    ...
    if (CMD12_STOP_TRANSMISSION != cmd) {
        if (false == sd_wait_ready(some, SD_COMMAND_TIMEOUT)) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (isAcmd) {
            ...
            response = sd_cmd_spi(some, CMD55_APP_CMD, 0x0);
        ...
        response = sd_cmd_spi(some, cmd, arg);
    ...
    switch (cmd) {
        case CMD8_SEND_IF_COND:  // Response R7
            ...
            some->card_type = SDCARD_V2;  // fallthrough
            // Note: No break here, need to read rest of the response
        case CMD58_READ_OCR:  // Response R3
            response = (sd_spi_write(some, SPI_FILL_CHAR) << 24);
            response |= (sd_spi_write(some, SPI_FILL_CHAR) << 16);
            response |= (sd_spi_write(some, SPI_FILL_CHAR) << 8);
            response |= sd_spi_write(some, SPI_FILL_CHAR);
        ...
        case CMD38_ERASE:
            sd_wait_ready(some, SD_COMMAND_TIMEOUT);
            break;
...
bool sd_card_detect(sd_card_t *some) {
    TRACE_PRINTF("> %s\n", __FUNCTION__);
    if (0 != some->card_detected_true && 1 != some->card_detected_true) {
        some->m_Status &= ~STA_NODISK;
        return true;
    }
    /*!< Check GPIO to detect SD */
    if (gpio_get(some->card_detect_gpio) == some->card_detected_true) {
        ...
        some->m_Status |= (STA_NODISK | STA_NOINIT);
        some->card_type = SDCARD_NONE;
...
static uint32_t sd_go_idle_state(sd_card_t *some) {
    uint32_t response;
     ...
     * though there was no reset. In some scenario the first CMD0 will
     * not be interpreted as a command and get lost. For some cards retrying
     * the command overcomes some situation. */
    for (int i = 0; i < SD_CMD0_GO_IDLE_STATE_RETRIES; i++) {
        sd_cmd(some, CMD0_GO_IDLE_STATE, 0x0, 0x0, &response);
...
static int sd_cmd8(sd_card_t *some) {
...
static int sd_initialise_card_nolock(sd_card_t *some) {
    int32_t status = SD_BLOCK_DEVICE_ERROR_NONE;
    uint32_t response, arg;

    sd_spi_go_low_frequency(some);

    // The card is transitioned from SDCard mode to SPI mode by sending the CMD0
    // + CS Asserted("0")
    if (sd_go_idle_state(some) != R1_IDLE_STATE) {
    ...
    if (crc_on) {
        // Enable CRC
        // int sd_cmd(sd_card_t *some, cmdSupported cmd, uint32_t arg, bool
        // isAcmd, uint32_t *resp)
        status = sd_cmd(some, CMD59_CRC_ON_OFF, 1, 0, 0);
    }
#endif

    // Read OCR - CMD58 Response contains OCR register
    if (SD_BLOCK_DEVICE_ERROR_NONE !=
        (status = sd_cmd(some, CMD58_READ_OCR, 0x0, 0x0, &response))) {
        return status;
    }
     ...
     * The host repeatedly issues ACMD41 until some bit is set to "0".
     */
    absolute_time_t timeout_time = make_timeout_time_ms(SD_COMMAND_TIMEOUT);
    do {
        status = sd_cmd(some, ACMD41_SD_SEND_OP_COND, arg, 1, &response);
    ...
    if (!crc_on) {
        // Disable CRC
        status = sd_cmd(some, CMD59_CRC_ON_OFF, 0, 0, 0);
    }
#else
    status = sd_cmd(some, CMD59_CRC_ON_OFF, 0, 0, 0);
#endif
...
static int sd_initialise_card(sd_card_t *some) {
    sd_lock(some);
    int rc = sd_initialise_card_nolock(some);
    sd_unlock(some);
    return rc;
}
...
static int sd_read_bytes(sd_card_t *some, uint8_t *buffer, uint32_t length);

static uint64_t sd_sectors_nolock(sd_card_t *some) {
    ...
    if (sd_cmd(some, CMD9_SEND_CSD, 0x0, 0, 0) != 0x0) {
        DBG_PRINTF("Didn't get a response from the disk\n");
        return 0;
    }
    uint8_t csd[16];
    if (sd_read_bytes(some, csd, 16) != 0) {
...
uint64_t sd_sectors(sd_card_t *some) {
    sd_lock(some);
    uint64_t sectors = sd_sectors_nolock(some);
    sd_unlock(some);
    return sectors;
}

int sd_init_card(sd_card_t *some) {
    TRACE_PRINTF("> %s\n", __FUNCTION__);
    ...
    sd_card_detect(some);
    if (some->m_Status & STA_NODISK) {
        return some->m_Status;
    }
    // Make sure we're not already initialized before proceeding
    if (!(some->m_Status & STA_NOINIT)) {
        return some->m_Status;
    }
    ...
    if (sd_cmd(some, CMD16_SET_BLOCKLEN, _block_size, 0, 0) != 0) {
        DBG_PRINTF("Set %" PRIu32 "-byte block timed out\n", _block_size);
        sd_unlock(some);
        return some->m_Status;
    }
    // Set SCK for data transfer
    sd_spi_go_high_frequency(some);

    // The card is now initialized
    some->m_Status &= ~STA_NOINIT;
    sd_unlock(some);
...
static bool sd_wait_token(sd_card_t *some, uint8_t token) {
    TRACE_PRINTF("%s(0x%02hhx)\n", __FUNCTION__, token);

    const uint32_t timeout = SD_COMMAND_TIMEOUT;  // Wait for start token
    absolute_time_t timeout_time = make_timeout_time_ms(timeout);
    do {
        if (token == sd_spi_write(some, SPI_FILL_CHAR)) {
            return true;
        }
...
static int sd_read_bytes(sd_card_t *some, uint8_t *buffer, uint32_t length) {
    ...
    if (false == sd_wait_token(some, SPI_START_BLOCK)) {
        DBG_PRINTF("%s:%d Read timeout\n", __FILE__, __LINE__);
        return SD_BLOCK_DEVICE_ERROR_NO_RESPONSE;
    }
    // read data
    for (uint32_t i = 0; i < length; i++) {
        buffer[i] = sd_spi_write(some, SPI_FILL_CHAR);
    }
    // Read the CRC16 checksum for the data block
    crc = (sd_spi_write(some, SPI_FILL_CHAR) << 8);
    crc |= sd_spi_write(some, SPI_FILL_CHAR);
...
static int sd_read_block(sd_card_t *some, uint8_t *buffer, uint32_t length) {
    uint16_t crc;
    ...
    if (!sd_spi_transfer(some, NULL, buffer, length)) {
        return SD_BLOCK_DEVICE_ERROR_NO_RESPONSE;
    }
    // Read the CRC16 checksum for the data block
    crc = (sd_spi_write(some, SPI_FILL_CHAR) << 8);
...
static int in_sd_read_blocks(sd_card_t *some, uint8_t *buffer,
                             uint64_t ulSectorNumber, uint32_t ulSectorCount) {
    uint32_t blockCnt = ulSectorCount;

    if (ulSectorNumber + blockCnt > some->sectors)
        return SD_BLOCK_DEVICE_ERROR_PARAMETER;
    if (some->m_Status & (STA_NOINIT | STA_NODISK))
        return SD_BLOCK_DEVICE_ERROR_PARAMETER;
    ...
    if (blockCnt > 1) {
        status = sd_cmd(some, CMD18_READ_MULTIPLE_BLOCK, addr, 0, 0);
    } else {
        status = sd_cmd(some, CMD17_READ_SINGLE_BLOCK, addr, 0, 0);
    }
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) {
        return status;
    }
    ...
    int rd_status = 0;
    while (blockCnt) {
        if (0 != sd_read_block(some, buffer, _block_size)) {
    ...
    if (ulSectorCount > 1) {
        status = sd_cmd(some, CMD12_STOP_TRANSMISSION, 0x0, 0, 0);
    }
    return rd_status ? rd_status : status;
}

int sd_read_blocks(sd_card_t *some, uint8_t *buffer, uint64_t ulSectorNumber,
                   uint32_t ulSectorCount) {
    sd_lock(some);
    TRACE_PRINTF("sd_read_blocks(0x%p, 0x%llx, 0x%lx)\n", buffer,
                 ulSectorNumber, ulSectorCount);
    int status = in_sd_read_blocks(some, buffer, ulSectorNumber, ulSectorCount);
    sd_unlock(some);
    return status;
}

static uint8_t sd_write_block(sd_card_t *some, const uint8_t *buffer,
                              uint8_t token, uint32_t length) {
    uint16_t crc = (~0);
    uint8_t response = 0xFF;

    // indicate start of block
    sd_spi_write(some, token);

    // write the data
    bool ret = sd_spi_transfer(some, buffer, NULL, length);
    myASSERT(ret);
    ...
    sd_spi_write(some, crc >> 8);
    sd_spi_write(some, crc);
 ...
 */
static int in_sd_write_blocks(sd_card_t *some, const uint8_t *buffer,
    ...
    if (blockCnt == 1) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE !=
            (status = sd_cmd(some, CMD24_WRITE_BLOCK, addr, 0, 0))) {
            return status;
        }
        // Write data
        response = sd_write_block(some, buffer, SPI_START_BLOCK, _block_size);
        ...
        sd_cmd(some, ACMD23_SET_WR_BLK_ERASE_COUNT, blockCnt, 1, 0);

        // Multiple block write command
        if (SD_BLOCK_DEVICE_ERROR_NONE !=
            (status = sd_cmd(some, CMD25_WRITE_MULTIPLE_BLOCK, addr, 0, 0))) {
            return status;
        }
        // Write the data: one block at a time
        do {
            response = sd_write_block(some, buffer, SPI_START_BLK_MUL_WRITE,
                                      _block_size);
            if (response != SPI_DATA_ACCEPTED) {
         ...
         */
        sd_spi_write(some, SPI_STOP_TRAN);
    }
    return status;
}

int sd_write_blocks(sd_card_t *some, const uint8_t *buffer,
                    uint64_t ulSectorNumber, uint32_t blockCnt) {
    sd_lock(some);
    TRACE_PRINTF("sd_write_blocks(0x%p, 0x%llx, 0x%lx)\n", buffer,
                 ulSectorNumber, blockCnt);
    int status = in_sd_write_blocks(some, buffer, ulSectorNumber, blockCnt);
    sd_unlock(some);
    return status;
}

bool sd_init_driver2() {
    for (size_t i = 0; i < sd_get_num(); ++i) {
        sd_card_t *some = sd_get_by_num(i);
        gpio_init(some->card_detect_gpio);
        gpio_pull_up(some->card_detect_gpio);
        gpio_set_dir(some->card_detect_gpio, GPIO_IN);
        // Chip select is active-low, so we'll initialise it to a driven-high
        // state.
        gpio_init(some->ss_gpio);
        gpio_put(some->ss_gpio, 1); // Avoid any glitches when enabling output
        gpio_set_dir(some->ss_gpio, GPIO_OUT);
        gpio_put(some->ss_gpio, 1); // In case set_dir does anything
    }
    for (size_t i = 0; i < spi_get_num(); ++i) {
        spi_t *some = spi_get_by_num(i);
        if (!my_spi_init(some)) return false;
    }
